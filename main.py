...
class File_Encoder(Frame):
    ...
    def submit(self):
        ...
        files = [os.path.join(os.getcwd(), self.file_list.get(f)) for f in files]
    ...
    def preview(self):
        ...
        if len(files) == 0:
            messagebox.showwarning("Warning", "Please select a file from the list.")
            return
        file_in_list = [os.path.join(os.getcwd(), self.file_list.get(f)) for f in files]
    ...
    def create_widgets(self):
        ...
        # Preview Button
        preview = Button(self, text="Preview", command=self.preview).grid(column=0, row=2)

        # Apply to All Checkbox
        # self.apply_all = Checkbutton(self, text="Apply to All", command=self.applyall).grid(column=2, row=0, sticky=NE)

        ## Output Location ##
        output_button = Button(self, text="Output Location", command=self.set_out_file).grid(column=2, row=2, padx=15)
        output_label = Label(self, textvariable=self.out_file_label).grid(column=2, row=1)
